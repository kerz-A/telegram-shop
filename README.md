...
### Админ-чат (группа)
При создании нового заказа бот автоматически отправляет в админ-группу сообщение с деталями и inline-кнопками для смены статуса:
...
### Команды для админов
В админ-группе доступна команда через ☰ Menu:

| Команда | Описание |
|---------|----------|
| `/active_orders` | Список активных (незавершённых) заказов |

Для доступа к команде пользователь должен быть отмечен как администратор в Django Admin → **Клиенты** → галочка **«Является администратором»**.
